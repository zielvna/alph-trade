...
import { balanceOf, getPositions, openPosition } from "../utils/web3";
...
export const SidePanel: React.FC = () => {
  ...
  const { balance, currentPrice, setBalance, setPositions } = useStore();
  ...
  const handleOpenPosition = async () => {
    if (signer && account) {
      const amount = BigInt(Number(colateral) * Number(TOKEN_DENOMINATOR));
  ...
  return (
    <div className="w-[360px] flex flex-col gap-4 p-4">
      ...
      <Button
        scheme={
          Number(colateral) > 0 &&
          BigInt(Number(colateral) * Number(TOKEN_DENOMINATOR)) <= balance
            ? Color.BLUE
            : Color.GRAY
        }
        size={ButtonSize.BIG}
